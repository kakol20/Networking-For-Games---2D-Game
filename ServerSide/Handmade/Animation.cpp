...
{
	//store total time elapsed since animation began which will be used in formula below
	//this has been hardcoded for now so that it works with a very empty engine
	//TODO: Change this so that it makes use of the game engine's delta time
	m_timeElapsed += 0.01f;
	...
	//frames will be called with the last texture cel set, so only
	//mark it dead when the first texture cel comes around again
