...
MainMenu::MainMenu()
{
	...
	TheTexture::Instance()->LoadFontFromFile("Assets/Fonts/Formula1-Regular.ttf", 200, "MENU_FONT");
}
//------------------------------------------------------------------------------------------------------
...
void MainMenu::SetMenuText(const std::string& text)
{
	...
	menuText.SetSize(text.size() * 30, 60);
...
void MainMenu::Update()
{
	...
	//loop through all menu items and set their initial color to dark green
	for (size_t i = 0; i < m_menuText.size(); i++)
	{
		m_menuText[i].SetColor(32, 96, 60);
	}

	//only set the active menu item to a white color
	m_menuText[m_menuOptionActive].SetColor(255, 255, 255);
}
//------------------------------------------------------------------------------------------------------
